...
const char *Rel_Date[]   =    { "  08-03-2022  "};
const char *Rel_Vers[]   =    { " V3.5 4 inch LCD "};
...
FM_Preset preset[] = {
  ...
  //  Please  modify this list according to your local Radio stations
...
void loop() {
  ...
  if (pressed) {
    ...
    if (FirstLayer) { //==================================================
      ...
      if (( x > 139) and (x < 219) and (y > 25) and (y < 60)) {  
...
void DisplayRDS()  {
  ...
  if (( currentMode == FM) and ((FirstLayer) or (ThirdLayer))) {
    if ( currentFrequency != previousFrequency ) {
      ...
      //   tft.fillRect(280, 120, 100, 30, TFT_BLACK);                                            
...
void showRSSI() {
  ...
  if ((  currentMode == FM ) and ((FirstLayer) or (ThirdLayer))) {
    ...
    //tft.fillRect(XFreqDispl + 350, YFreqDispl + 90 , 25, 10, TFT_RED);                              
    tft.drawString(buffer, XFreqDispl + 380, YFreqDispl + 50);
...
bool checkStopSeeking() {
  ...
  return (bool) encoderCount || tft.getTouch(&x, &y); 
}
...
void DrawDispl() {
  ...
  if (FirstLayer or ThirdLayer) {
    ...
    tft.setCursor(30, 50);
    tft.println("yo2ldk-Op. Alex");                       // edit this with your callsign and name !!
...
void AGCfreqdisp() {
  ...
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  tft.setTextDatum(BC_DATUM);
  tft.drawString("AGC", XFreqDispl - 20, YFreqDispl - 76);          //   -25  -25                     
  si4735.getAutomaticGainControl();
  if (si4735.isAgcEnabled()) {
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.drawString("On", XFreqDispl - 19, YFreqDispl - 58);          //   -25   -5                    
