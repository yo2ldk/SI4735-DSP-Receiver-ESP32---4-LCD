...
int Ybutst  = 230 + ytotoffset;  // Y Start location Buttons

int Xsmtr   =   230;
int Ysmtr   =  3;              //+ ytotoffset  //      POZITIE  Smeter 85

int XVolInd =  255;
int YVolInd = 45;           // + ytotoffset;  //       POZITIE  Volume indicator

int XFreqDispl  =   50;
int YFreqDispl  =   100 + ytotoffset;  // display

int Xbutsiz =  80;  //size of buttons first & third layer //80
int Ybutsiz =  43;  // 40
...
void Smeter() {
  ...
  // POZITIE SEGMENTE S-METRU
  tft.fillRect(Xsmtr + 45, Ysmtr + 30 , (3 + spoint), 5, TFT_GREEN); // riga rossa per i led dello smeter (da 0 a 4 )

  if (spoint > 45) {
    tft.fillRect(Xsmtr + 93, Ysmtr + 28 , (3 + spoint - 40), 7, TFT_ORANGE); // riga arancione per i led dello smeter ( da 4 a 9)
  }
  if (spoint > 105) {
    tft.fillRect(Xsmtr + 155, Ysmtr + 25 , (3 + spoint - 60), 11, TFT_RED); // riga verde per i led dello smeter ( da 9 a 9+60)
  }

  for (int i = 0; i < 19; i++) // numero righe trasversali nere per i led smeter
    tft.fillRect(Xsmtr + 35 + (i * 12), Ysmtr + 25, 3, 10, TFT_BLACK); // traccia righe trasversali nere per i led smeter spaziate di 12 pixel
  tft.fillRect(Xsmtr + 37 + spoint, Ysmtr + 25 , 226 - (2 + spoint), 11, TFT_BLACK);  // pulisce il fondo nero dello smeter
}
...
void loop() {
  ...
  if (currentPRES != previousPRES)
  {
    ...
    DrawDispl();
    tft.fillRect( 20, 90 , 350, 80, TFT_BLACK);                                                                           // Black freq. field preset
    AGCfreqdisp();
    tft.setTextColor(TFT_WHITE, TFT_BLACK );
    tft.setTextSize(3);                                                                                                   // dimensiune font preset
    tft.setTextDatum(BC_DATUM);

    tft.drawString(String(currentPRES) + ") " + String(((preset[currentPRES].presetIdx) / 100), 1) + " MHz", 160, 160);   // pozitie frecventa preset
    tft.setTextColor(TFT_YELLOW, TFT_BLACK );
    tft.drawString(String(preset[currentPRES].PresetName), 200, 120);                                                     // pozitie nume preset
    bandIdx = 0;
...
void DisplayRDS()  {
  ...
  if (( currentMode == FM) and ((FirstLayer) or (ThirdLayer))) {
    if ( currentFrequency != previousFrequency ) {
      previousFrequency = currentFrequency;
      tft.fillRect(XFreqDispl + 70, YFreqDispl + 65, 120, 35, TFT_BLACK);                     //  RDS CLEAR
      //tft.fillRect(0, YFreqDispl + 78, 240, 17, TFT_BLACK);// clear RDS rdsMsg  ANTONINO
      //   tft.fillRect(280, 120, 100, 30, TFT_BLACK);                                            // clear RDS rdsTime   ANTONINO

    }
    if ((RDS) and  (NewSNR >= 12)) checkRDS();    // 9 ERA 12
    else  tft.fillRect(XFreqDispl + 70, YFreqDispl + 70, 120, 40, TFT_BLACK); // clear RDS text                 //  RDS CLEAR
  }
}
...
void showRSSI() {
  ...
  //Display on FM mode Stereo or Mono

  if ((  currentMode == FM ) and ((FirstLayer) or (ThirdLayer))) {
    sprintf(buffer, "%s", (si4735.getCurrentPilot()) ? "STEREO" : "MONO");                                //  POZITIE INDICATOR FM   STEREO / MONO
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.setTextSize(2);
    tft.setTextDatum(BC_DATUM);
    tft.setTextPadding(2);
   tft.fillRect( XFreqDispl + 320, YFreqDispl-10 , 120, 40, TFT_BLACK);                                   // CURATA LOC STEREO MONO 
    tft.drawString(buffer, XFreqDispl + 370, YFreqDispl + 20);

    //4 RDS ANOTONINO
    /*    tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
       tft.setTextSize(2);
       tft.fillRect(30, 120 , 30, 25, TFT_BLACK);
       tft.drawString(" FM", 30, 120);
      }


      if ((band[bandIdx].bandType == MW_BAND_TYPE) and ((FirstLayer) or (ThirdLayer))and ( currentMode == AM )) {
       tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
       tft.setTextSize(2);
       tft.fillRect(30, 120 , 30, 25, TFT_BLACK);
       tft.drawString(" MW", 30, 120);
      }
      if ((band[bandIdx].bandType == LW_BAND_TYPE) and ((FirstLayer) or (ThirdLayer))and ( currentMode == AM )){
        tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
       tft.setTextSize(2);
       tft.fillRect(30, 120 , 30, 25, TFT_BLACK);
       tft.drawString(" LW", 30, 120);
      }

      if ((band[bandIdx].bandType == SW_BAND_TYPE) and ((FirstLayer) or (ThirdLayer))and ( currentMode == AM )){
        tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
       tft.setTextSize(2);
       tft.fillRect(30, 120 , 30, 25, TFT_BLACK);
       tft.drawString(" SW", 30, 120);
    */
  }
  ...
  if ((FirstLayer) or (ThirdLayer)) {  // dBuV and dB at freq. display
    ...
    tft.fillRect(5, 215 , 300, 20, TFT_BLACK);                                    // CURATARE LOC RSSI
    tft.setTextDatum(TL_DATUM);
    tft.drawString("RSSI = " + String(NewRSSI) + "dBuV" , 5,  215);               // RSSI
    tft.setTextDatum(TR_DATUM);
    tft.drawString("SNR = " + String(NewSNR) + "dB", 305, 215);                   // SNR
  }
  //ANTONINO
...
void DrawVolumeIndicator()  {
  ...
  /*
    ...
    tft.print("0%");
    tft.setCursor(XVolInd + 107, YVolInd + 7);
    tft.print("50%");
    tft.setCursor(XVolInd + 180, YVolInd + 7);
    tft.print("100%");
    }
  */
  // ANTONINO
...
void Steplist() {
  ...
  if ( currentMode != FM) {
    for (int n = 0 ; n <= lastStep; n++) {
      ...
      spr.setTextColor(COLOR_PANEL_TEXT);
      spr.setTextSize(2);
      spr.setTextDatum(BC_DATUM);                                                                                       // steps for  AM
    ...
    for (int n = 0 ; n <= 2; n++) {
      ...
      spr.setTextDatum(BC_DATUM);
      spr.setTextPadding(0);
      spr.drawString(String((sp[n].stepFreqFM) * 10, 0) + "kHz", But_Mode_Width / 2, But_Mode_Height - 2);             // steps for  FM
      spr.pushSprite((sp[n].Xstepos) + 50, sp[n].Ystepos + 5 + sp[n].Ystepnr);
...
void BWList()  {
  ...
  for (int n = 0 ; n < nrbox; n++) {
    ...
    spr.pushSprite((bw[n].Xos + 50), bw[n].Yos + 40 + (bw[n].Ynr - 20));                                              // pozitie butoane bandwidth
  }
  spr.deleteSprite();
  ...
  if ( currentMode == AM)  tft.drawString("AM Filter in kHz"  , XfBW + 120, YfBW + 280 );                             // pozitie text frecv bandwith
...
void showRDSStation() {
  ...
  if ((FirstLayer) or (ThirdLayer)) {
    tft.setCursor(XFreqDispl + 70, YFreqDispl + 78);                                                //  RDS TEXT
...
/*
  //=======================================================================================
  void showrdsMsg()    //ANTONINO
  {
  //=======================================================================================
  if ((FirstLayer) or (ThirdLayer)) {
    tft.setCursor(5, 165); //sulle coord di DSP radio...
    tft.print(rdsMsg);
  }
  delay(150);
  }
  ...
  if ((FirstLayer) or (ThirdLayer)) {
    ...
    tft.setCursor(315, 80); //sulle coord di rdsstation                                         // POZITIE CEAS RDS
    tft.print(rdsTime);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.setTextDatum(BC_DATUM);
...
void checkRDS() {
  ...
  if (si4735.getRdsReceived()) {
    if (si4735.getRdsSync() && si4735.getRdsSyncFound() ) {
      ...
      //rdsMsg = si4735.getRdsText2A();                                                                      // testo rds news  ANTONINO
      //rdsTime = si4735.getRdsTime();                                                                         // orario           ANTONINO
      tft.setTextSize(2);
      tft.setTextColor(TFT_YELLOW, TFT_BLACK);
      tft.setTextDatum(BC_DATUM);
      if ( stationName != NULL )   showRDSStation();                                                         // RDSS DENUMIRE POST
      ...
      /*
        tft.setTextSize(2);
           ...
           if ( rdsMsg != NULL )   showrdsMsg();                     //ANTONINO
...
void FreqDispl() {
  ...
  if ((FirstLayer) or (ThirdLayer)) {
    currentFrequency = si4735.getFrequency();
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.fillRect( XFreqDispl + 240, YFreqDispl - 35, 170, 30, TFT_BLACK);                            // clean volume display
    
    tft.fillRect( 20, 90 , 350, 80, TFT_BLACK);                                                      // clear freq display after preset
    ...
    if ((VOLbut) or (AGCgainbut)) {
      if (VOLbut) {
        tft.setTextSize(3);
        tft.setTextColor(TFT_YELLOW, TFT_BLACK);
        tft.drawString(String(map(currentVOL, 20, 63, 0, 100)), XFreqDispl + 380, YFreqDispl - 7);     // afisare % volum (0 -- 100%)
        tft.setTextSize(2);
        tft.drawString( "Volume", XFreqDispl + 285, YFreqDispl - 7);                                   // afisare pozitie  "volume"
      ...
      if ((band[bandIdx].bandType == MW_BAND_TYPE || band[bandIdx].bandType == LW_BAND_TYPE) and (bfoOn == false)) {
        ...
        tft.drawString(String(Displayfreq, 0), XFreqDispl + 120, YFreqDispl + 54);                     // freq mw
        tft.setTextSize(2);
        tft.setTextColor(TFT_WHITE, TFT_BLACK);
        tft.setFreeFont(NULL);
        tft.drawString("KHz", XFreqDispl + 250, YFreqDispl + 54);                                      // khz MW
        /*  } */
      }
      if (band[bandIdx].bandType == FM_BAND_TYPE) {
        ...
        tft.setTextColor(COLOR_INDICATOR_FREQ, COLOR_BACKGROUND);
        tft.setFreeFont(&DSEG7_Classic_Mini_Bold_Italic_52);
        tft.drawString(String(Displayfreq, 2), XFreqDispl + 120, YFreqDispl + 54);                        // pozitie freq FM
        tft.setTextSize(2);
        tft.setTextColor(COLOR_INDICATOR_FREQ, TFT_BLACK);
        tft.setFreeFont(NULL);
        tft.drawString("MHz", XFreqDispl + 250, YFreqDispl + 54);                                          // pozitie mhz
      }
      if ((currentMode == AM) and (band[bandIdx].bandType != MW_BAND_TYPE) and (band[bandIdx].bandType != LW_BAND_TYPE)) {
        ...
        tft.setTextColor(COLOR_INDICATOR_FREQ, COLOR_BACKGROUND);
        tft.setTextDatum(BC_DATUM);
        tft.setFreeFont(&DSEG7_Classic_Mini_Bold_Italic_52);                                                     //font frecv
        tft.drawString(String(Displayfreq, 3), XFreqDispl + 110, YFreqDispl + 54);                           // pozitie freq FM
        tft.setTextSize(2);
        tft.setTextColor(COLOR_INDICATOR_FREQ, TFT_BLACK);
        tft.setFreeFont(NULL);
        tft.drawString("MHz", XFreqDispl + 270, YFreqDispl + 54);                                             // pozitie MHz
      }
      if (currentMode == LSB || currentMode == USB  || currentMode == CW ) {                                        // HAM position ssb
        if (bfoOn) {
          tft.setTextSize(1);
          tft.setTextColor(COLOR_INDICATOR_FREQ, COLOR_BACKGROUND);
          tft.setTextDatum(BR_DATUM);
          tft.setFreeFont(&DSEG7_Classic_Mini_Bold_20);                                                            //font bfo
#ifdef IhaveCrystal
          tft.drawString(String(currentBFOmanu), XFreqDispl + 240, YFreqDispl + 54);                               // pozitie bfo
          //#endif
          //#ifdef IhaveSI5351
          ...
          tft.setTextSize(2);
          tft.setTextColor(COLOR_INDICATOR_FREQ, TFT_BLACK);
          tft.drawString("Hz", XFreqDispl + 300, YFreqDispl + 54);                                                // pozitie hz bfo
          //#ifdef IhaveSI5351
          ...
          //tft.fillRect( XFreqDispl + 285, YFreqDispl , 115, 25, TFT_BLACK); // Black freq. field
          //tft.fillRect( XFreqDispl + 350, YFreqDispl + 54 , 100, 25, TFT_BLACK); // Black freq. field
          ...
          tft.drawString(String(s), XFreqDispl + 260, YFreqDispl + 54);                                               // pozitie freq SSB
          tft.setTextSize(2);
          tft.setTextColor(COLOR_INDICATOR_FREQ, TFT_BLACK);
          tft.setFreeFont(NULL);
          tft.drawString("MHz", XFreqDispl + 300, YFreqDispl + 54);                                                  // pozitie nhz SSB
          if (freqstepnr == 0)  tft.fillRect(XFreqDispl + 220, YFreqDispl + 54, 20, 5, COLOR_INDICATOR_FREQ);
          if (freqstepnr == 1)  tft.fillRect(XFreqDispl + 230, YFreqDispl + 54, 20, 5, COLOR_INDICATOR_FREQ);         // pozitie freq SSB
...
void SeekFreq (uint16_t freq)  {
  ...
  if ((FirstLayer) or (ThirdLayer))  {
    ...
    if (band[bandIdx].bandType == SW_BAND_TYPE)  {
      ...
      tft.setTextDatum(BC_DATUM);
      tft.setFreeFont(&DSEG7_Classic_Mini_Bold_Italic_52);
      tft.drawString(String(Displayfreq, 3), XFreqDispl + 260, YFreqDispl + 54);            //scan ssb
      tft.setTextSize(2);
      tft.setTextColor(COLOR_INDICATOR_FREQ, TFT_BLACK);
      tft.setFreeFont(NULL);
...
void DrawDispl() {
  ...
  if (FirstLayer or ThirdLayer) {
    ...
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.setTextDatum(BC_DATUM);
    tft.drawString(band[bandIdx].bandName, XFreqDispl + 40, YFreqDispl - 58);        // BAND NAME
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Band", XFreqDispl + 40, YFreqDispl - 76);                        // BAND

    FreqDispl();
    //if (band[bandIdx].bandType != FM_BAND_TYPE) {
    tft.setTextSize(2);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Mod.", XFreqDispl + 100, YFreqDispl - 76);                       // MOD
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    ...
    tft.drawString(Modtext, XFreqDispl + 98, YFreqDispl - 58);                                                                    //  USB,LSB,CW..
    ...
    if (BWtext == "AUT") {
      tft.drawString(BWtext, 410, 232);                                                                                            // "AUT"
    } else tft.drawString(BWtext + " KHz", 435, 232);                                                                              // BAND WIDTH KHZ
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Width", 350, 232);                                                                                             //  WIDTH
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    if (currentMode == FM) {
      tft.drawString(String((band[bandIdx].currentStep) * 10) + "KHz", XFreqDispl + 157, YFreqDispl - 56);                       // STEP SIZE
    } else  tft.drawString(String(band[bandIdx].currentStep) + "KHz", XFreqDispl + 157, YFreqDispl - 56);                        // STEP SIZE
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Step", XFreqDispl + 157, YFreqDispl - 76);                                                                    // STEP
...
void AGCfreqdisp() {
  ...
  tft.setTextColor(TFT_GREEN, TFT_BLACK);
  tft.setTextDatum(BC_DATUM);
  tft.drawString("AGC", XFreqDispl - 20, YFreqDispl - 76);          //   -25  -25                      //  AGC
  si4735.getAutomaticGainControl();
  if (si4735.isAgcEnabled()) {
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.drawString("On", XFreqDispl - 19, YFreqDispl - 58);          //   -25   -5                     //  STATUS
    tft.setTextColor(TFT_ORANGE, TFT_BLACK);
  } else {
    if (AGCgain == 0)   {
...
void BFOStepdisp() {
  ...
  if (band[bandIdx].bandType != FM_BAND_TYPE) {
    ...
    //tft.drawString("BFO", XFreqDispl + 140, YFreqDispl - 76);                                              // afiseaza "bfo"
    tft.setTextPadding(tft.textWidth("88"));

    if (bfoOn) {
#ifdef IhaveCrystal
      tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    //  tft.drawString(String(currentBFOStep), XFreqDispl + 140, YFreqDispl - 58);                           // afiseaza step "bfo"
      tft.setTextColor(TFT_GREEN, TFT_BLACK);
#endif
    } else {
      tft.setTextColor(TFT_GREEN, TFT_BLACK);
