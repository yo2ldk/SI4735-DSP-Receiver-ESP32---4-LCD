...
void VolumeIndicator(int vol) {
  //=======================================================================================
  vol = map(vol, 20, 63, 0, 183);
  tft.fillRect(XVolInd + 22, YVolInd + 16 , (2 + vol), 6, TFT_GREEN);
  tft.fillRect(XVolInd + 22 + vol, YVolInd + 16 , 183 - (2 + vol), 6, TFT_VIOLET);
}

//=======================================================================================
void brightnessIndicator(int vol) {
  //=======================================================================================
  vol = map(vol, 250, 20, 0, 183);
  tft.fillRect(XVolInd + 22, YVolInd + 16 , (2 + vol), 6, TFT_RED);
  tft.fillRect(XVolInd + 22 + vol, YVolInd + 16 , 183 - (2 + vol), 6, TFT_GREEN);
}
...
void Dispoff()  {
  //=======================================================================================
  if (((millis() - DisplayOnTime) > MIN_ELAPSED_DISPL_TIME * 1000) and (DISplay == true)) {
    DISplay = false;
...
void VOLbutoff()  {
  //=======================================================================================
  if (((millis() - VOLbutOnTime) > MIN_ELAPSED_VOLbut_TIME * 20) and (VOLbut == true)) {
    VOLbut = false;
    drawVOL();
    FreqDispl();
  }
...
void DisplayRDS()  {
  ...
  if (( currentMode == FM) and ((FirstLayer) or (ThirdLayer))) {
    if ( currentFrequency != previousFrequency ) {
      previousFrequency = currentFrequency;
      tft.fillRect(XFreqDispl + 20, YFreqDispl + 62, 400, 40, TFT_BLACK);                                                          
    }
    if ((RDS) and  (NewSNR >= 5)) checkRDS(); 
    else  tft.fillRect(XFreqDispl + 50, YFreqDispl + 66, 150, 35, TFT_BLACK);           
  }
}
...
void showRSSI() {
  ...
  if ((FirstLayer) or (ThirdLayer)) {  // dBuV and dB at freq. display
    ...
    tft.drawString("RSSI =" , 43,  232);
    tft.drawString("SNR =" , 207,  232);
                                     
    tft.setTextDatum(TL_DATUM);
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.drawString( String(NewRSSI) + "dBuV" , 86,  216);            
    tft.setTextDatum(TR_DATUM);
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    tft.drawString(String(NewSNR) + "dB", 292, 216);                  
  }
  VOLbutoff();
}
...
void showRDSStation() {
  ...
  if ((FirstLayer) or (ThirdLayer)) {
    tft.setCursor(XFreqDispl + 90, YFreqDispl + 65);                                           
...
//=======================================================================================
  if ((FirstLayer) or (ThirdLayer)) {
    tft.setCursor(85, YFreqDispl + 93); 
    tft.print(rdsMsg);
  }
  delay(150);
}
...
if ((FirstLayer) or (ThirdLayer)) {
    ...
    tft.setCursor(385, YFreqDispl + 93);
    tft.print(rdsTime);
    tft.setTextSize(2);
    tft.setTextColor(TFT_SKYBLUE, TFT_BLACK);
    tft.setTextDatum(BC_DATUM);
    tft.setCursor(395, YFreqDispl + 65);
      tft.print("Time");
  }
  delay(150);
}
...
void DrawDispl() {
  ...
  if (FirstLayer or ThirdLayer) {
    ...
    } else tft.drawString(BWtext + " KHz", 425, 232);                                                                            
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Width:", 343, 232);                                                                                           
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    if (currentMode == FM) {
